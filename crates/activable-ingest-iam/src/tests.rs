...
fn stmt_with_not_action(effect: Effect, not_actions: &[&str], resources: &[&str]) -> PolicyStatement {
    PolicyStatement {
        sid: None,
        effect,
        actions: vec![],
        not_actions: not_actions.iter().map(|a| ActionPattern(a.to_string())).collect(),
        resources: resources.iter().map(|r| ResourcePattern(r.to_string())).collect(),
        not_resources: vec![],
        conditions: vec![],
    }
...
fn deny_with_resource_pattern() {
    ...
    let result1 = evaluate_deny(&[stmt_deny.clone()], "s3:GetObject", "arn:aws:s3:::secret-data");
...
fn deny_with_matching_condition() {
    ...
    deny.conditions
        .push(condition("StringNotEquals", "aws:RequestedRegion", &["us-east-1"]));
...
fn deny_with_multiple_conditions_all_must_match() {
    let mut deny = stmt(Effect::Deny, &["s3:*"], &["*"]);
    deny.conditions
        .push(condition("StringEquals", "aws:RequestedRegion", &["us-east-1"]));
    deny.conditions
...
fn string_equals_match() {
    assert!(evaluate_condition("StringEquals", "aws:RequestedRegion", &["us-east-1"], "us-east-1"));
}
...
fn string_like_wildcard() {
    assert!(evaluate_condition("StringLike", "s3:prefix", &["home/*"], "home/alice/doc.txt"));
}

#[test]
fn string_like_no_match() {
    assert!(!evaluate_condition("StringLike", "s3:prefix", &["home/*"], "work/bob/file.txt"));
}
...
fn string_not_like_no_match() {
    assert!(!evaluate_condition("StringNotLike", "s3:prefix", &["home/*"], "home/alice/doc.txt"));
}
...
fn null_condition_true() {
    assert!(evaluate_condition("Null", "aws:MultiFactorAuthAge", &["true"], ""));
}

#[test]
fn null_condition_false() {
    assert!(!evaluate_condition("Null", "aws:MultiFactorAuthAge", &["true"], "3600"));
}

#[test]
fn null_condition_false_with_false_value() {
    assert!(evaluate_condition("Null", "aws:MultiFactorAuthAge", &["false"], "3600"));
}

#[test]
fn bool_condition_true() {
    assert!(evaluate_condition("Bool", "aws:SecureTransport", &["true"], "true"));
}

#[test]
fn bool_condition_false() {
    assert!(!evaluate_condition("Bool", "aws:SecureTransport", &["true"], "false"));
}

#[test]
fn bool_condition_false_value() {
    assert!(evaluate_condition("Bool", "aws:SecureTransport", &["false"], "false"));
}
