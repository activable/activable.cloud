...
impl NativeEnricher for Ec2Enricher {
    ...
    async fn enrich(
        ...
        if !edges.is_empty() {
            ...
            let outcome =
                load_edges(pool.clone(), graph_name, "HasSecurityGroup", &edges, 100, false).await?;
            debug!(created = outcome.created, dropped = outcome.dropped, "HasSecurityGroup edges outcome");
