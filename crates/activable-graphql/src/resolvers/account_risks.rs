...
pub async fn account_risks(
    ...
    if principal_ids.is_empty() {
        return Err(async_graphql::Error::new("Account not found or has no principals"));
    }
    ...
    let sub_scores = vec![
        category_max_scores.get("cf_escalation").copied().unwrap_or(0.0),
        category_max_scores.get("oidc_drift").copied().unwrap_or(0.0),
        category_max_scores.get("s3_boundary").copied().unwrap_or(0.0),
    ...
    let top_principals: Vec<GqlPrincipalRisk> = principal_risks
        .into_iter()
        .take(limit)
        .map(|(principal_id, score, severity, matched_rules)| GqlPrincipalRisk {
            principal_id,
            score,
            severity,
            matched_rules,
